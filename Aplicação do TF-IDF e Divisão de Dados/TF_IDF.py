...
def carregar_e_preparar_dados(caminho_arquivo: str) -> Tuple[pd.Series, np.ndarray, np.ndarray]:
    """Carrega o dataset, filtra as classes e retorna as features e labels."""
...
def obter_pipelines() -> Dict[str, Pipeline]:
    ...
    modelos = {
        ...
        "KNN (5 Vizinhos)": KNeighborsClassifier(n_neighbors=5, n_jobs=1)
    }
...
def salvar_matriz_confusao(modelo: Pipeline, X_train: pd.Series, y_train: np.ndarray, 
    ...
    plt.title(f'Matriz de Confusão - {nome_modelo} ({nome_split})')
    
    nome_limpo = nome_modelo.replace(' ', '_').replace('(', '').replace(')', '')
    caminho_imagem = os.path.join(pasta_resultados, f"Matriz_Confusao_{nome_limpo}_{nome_split}.png")
...
def main():
    ...
    metricas = ['accuracy', 'recall_weighted', 'f1_macro']
    ...
    for treino_pct, val_pct, teste_pct in configuracoes_splits:
        ...
        with open(caminho_txt, 'w', encoding='utf-8') as arquivo_txt:
            ...
            for nome_modelo, pipeline in pipelines.items():
                print(f"Rodando K-Fold para {nome_modelo}... ")
                
                inicio_tempo = time.time()
                scores = cross_validate(pipeline, X_train, y_train, cv=skf, scoring=metricas, n_jobs=-1)
                tempo_total = time.time() - inicio_tempo
                
                acc_media = np.mean(scores['test_accuracy'])
                recall_medio = np.mean(scores['test_recall_weighted'])
                f1_macro_medio = np.mean(scores['test_f1_macro'])
                
                resultados_kfold.append({
                    "Modelo": nome_modelo,
                    "Acurácia": acc_media,
                    "Recall (Weighted)": recall_medio,
                ...
                texto_resultado = (
                    ...
                    f"Recall Médio: {recall_medio:.6f}\n"
